...
- (void)viewDidLoad {
    ...
    if (![self isIpad]) {
        ...
        //  因为原写法键盘按钮位置太小 显示不出来 更改成自己写的button
        // Use a toolbar so that we can tint
//        UIToolbar *keyboardToolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(-7, -1, 44, 44)];
//        keyboardToolbar.backgroundColor = MAIN_ARM_COLOR;
//        [toolbarCropper addSubview:keyboardToolbar];
//        self.keyboardItem = [[ZSSBarButtonItem alloc]initWithCustomView:keyBoardBtn];
...
//        keyboardToolbar.items = @[self.keyboardItem];
        ...
        keyBoardBtn.frame = CGRectMake(0, -3, 44, 44);
        ...
        [toolbarCropper addSubview:keyBoardBtn];
...
- (void)buildToolbar {
    ...
    if (IPHONE6) {
        toolbarWidth = items.count == 0 ? 0.0f : (CGFloat)(items.count * 41);
    }else if (IPHONE6P){
        toolbarWidth = items.count == 0 ? 0.0f : (CGFloat)(items.count * 47);
    }
